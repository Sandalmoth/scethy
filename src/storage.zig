...
// greatly improving the result when splitting buckets
...
pub const DataStorage = struct {
    ...
    pub fn copy(storage: *DataStorage, comptime V: type) DataStorage {
        ...
        for (0..PAGE_INDEX_SIZE) |i| {
            ...
            if (storage.page_index.pages[i].?.head.modified) {
    ...
    pub fn del(storage: *DataStorage, comptime V: type, key: Entity) bool {
        ...
        storage.page_index.pages[loc.page].?.head.modified = true;
        storage.page_index.pages[last_loc.page].?.head.modified = true;
